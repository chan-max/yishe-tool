...
registerOperation({
  ...
  name: '复制元素',
  description: '复制指定元素，创建一个副本。可以指定偏移位置避免重叠',
  group: '元素',
  params: [
    {
      name: 'id',
  ...
  execute(params, ctx) {
    ...
    if (newId && (offsetX !== 0 || offsetY !== 0)) {
      ...
      if (newChild?.position) {
        const currentLeft = typeof newChild.position.left === 'object'
          ? newChild.position.left.value
          : newChild.position.left || 0
        const currentTop = typeof newChild.position.top === 'object'
