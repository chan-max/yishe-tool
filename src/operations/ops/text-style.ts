...
registerOperation({
  ...
  execute(params, ctx) {
    ...
    if (child.type !== 'text') return { success: false, message: `元素 ${id} 不是文字类型` }
    ctx.setChildProperty(id, 'fontColor', color)
    return { success: true, message: `已将文字颜色设置为 ${color}`, data: { id, color } }
  },
})
...
registerOperation({
  ...
  execute(params, ctx) {
    ...
    return { success: true, message: `已将字号设置为 ${fontSize}`, data: { id, fontSize } }
  },
})
...
registerOperation({
  ...
  execute(params, ctx) {
    ...
    return { success: true, message: `已将文字粗细设置为 ${fontWeight}`, data: { id, fontWeight } }
  },
})
