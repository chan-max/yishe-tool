                            ...
                            <template v-for="item, index in model">
                                <el-col :span="24">
                                    <div class="flex items-center justify-between" style="column-gap:1rem">
                                        ...
                                        水平偏移:
                                        <size-input :unit-options="unitOptions" v-model="model[index].horizontal.value"
                                            v-model:unit="model[index].horizontal.unit"></size-input>
                                        垂直偏移:
                                        <size-input :unit-options="unitOptions" v-model="model[index].vertical.value"
                                            v-model:unit="model[index].horizontal.unit"></size-input>
                                        模糊半径:
                                        <size-input :unit-options="unitOptions" v-model="model[index].blur.value"
                                            v-model:unit="model[index].horizontal.unit"></size-input>
                                        颜色:
                                        <color-picker type="pure" v-model="model[index].color"></color-picker>
                                        隐藏：
                                        <el-switch size="small" v-model="model[index].disabled"></el-switch>
                                        <el-button type="danger" plain size="small" @click="remove(index)"> 移除
                                        </el-button>
                                    </div>
...
import { getPositionInfoFromOptions, formatToNativeSizeOption, parseTextShadowOptionsToCSS } from '@/components/design/layout/canvas/helper.tsx'
