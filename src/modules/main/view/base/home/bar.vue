<template>
  <el-carousel
    style="border-radius: 8px; overflow: hidden; width: 92%; height: 100px;box-sizing:border-box;"
    direction="vertical"
    :motion-blur="true"
  >
    <el-carousel-item>
      <div class="bar">
        <div></div>
        <div>
          <el-button class="button-try" plain @click="$router.push({ name: 'Design' })">
            快速开始
          </el-button>
        </div>
      </div>
    ...
    <el-carousel-item>
      <div class="bar">
        <div></div>
        <div>
          <el-button class="button-try" plain @click="dialogVisible = true">
            上传资源
          </el-button>
        </div>
      </div>
  ...
  <el-dialog v-model="dialogVisible" title="资源上传" width="680" @close="close">
    ...
    >
      ...
      <template #file="{ file }">
        <div class="file-bar">
          <div>
            <el-icon size="24"><component :is="fileTypeIcons[getFileSuffix(file.name)]"></component></el-icon>
          </div>
...
import { UploadFilled, CircleCloseFilled, QuestionFilled } from "@element-plus/icons-vue";
