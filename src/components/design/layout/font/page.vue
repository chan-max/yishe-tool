<template>
    <div class="flex">
        <div style="width:360px;height:540px;padding:1rem;row-gap:1rem;overflow:auto;" class="flex flex-col ">
            <div class="label"> {{ activeFont.name }} 预览 </div>
            <div :style="{ fontSize: previewFontSize + 'px', fontFamily: `font_${activeFont.id}` }"
                class="flex items-center justify-center"
                style="width:100%;height:240px;background:rgba(115, 0, 255, .05);overflow:hidden">
            ...
            <div class="flex flex-wrap" style="gap:1rem .5rem;">
        ...
        <div style="width:720px;">
            <div style="padding:1rem;" class="flex items-center justify-between">
                ...
                <el-button size="small" link :icon="TopRight" @click="goUpload"> 去上传 </el-button>
                <el-button size="small" link :icon="TopRight" @click="goMine"> 查看我的上传 </el-button>
            </div>
            <s1-scrollbar height="540px">
                <div v-infinite-scroll="getList" :infinite-scroll-distance="150" style="padding:1rem;">
                    <div v-for="item in list" class="item" @click="select(item)">
                        ...
                        <s1-image :src="item.thumbnail" fit="contain" style="width:auto;height:48px;"></s1-image>
                    </div>
...
import { operatingTextStickerOptions, showFontModal, showUpload,viewDisplayController } from "../../store";
...
// 字体列表
const { list, getList, reset, loading, total } = usePaging(
    (params) => {
