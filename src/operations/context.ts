...
function getCanvasChild() {
  return canvasStickerOptions.value.children.find((c: any) => c.type === 'canvas')
}
...
export function createDesignOperationContext(): OperationContext {
  return {
    getCanvasSize() {
      ...
      return { width: child.width.value, height: child.height.value, unit: child.width.unit }
    },
    ...
    getCanvasBackgroundColor() {
      const bg = canvasStickerOptions.value.supportBackgroundColor
      return bg?.color || 'rgba(0,0,0,0)'
    },

    setCanvasBackgroundColor(color) {
      canvasStickerOptions.value.supportBackgroundColor = {
        type: 'pure',
        color,
      }
    },
    ...
    addCanvasChild(type, options = {}) {
      ...
      const lastChild = canvasStickerOptions.value.children[canvasStickerOptions.value.children.length - 1]
      return lastChild?.id || ''
    },
    ...
    getChildProperty(id, propPath) {
      const child = canvasStickerOptions.value.children.find((c: any) => c.id === id)
