...
registerOperation({
  ...
  execute(params, ctx) {
    const { content, qrCodeColor, backgroundColor, width, height, dotType, errorCorrectionLevel, center } = params

    const extraOptions: Record<string, any> = {}

    if (content !== undefined) extraOptions.qrcodeContent = content
    if (qrCodeColor !== undefined) extraOptions.qrCodeColor = { color: qrCodeColor, type: 'pure' }
    ...
    if (errorCorrectionLevel !== undefined) extraOptions.errorCorrectionLevel = errorCorrectionLevel
