<template>
  <div
    ...
    class="v3ti"
  >
    <div class="v3ti-content" ref="inputBox" :class="{ 'v3ti-content--select': select }">
      <span v-for="(tag, index) in innerTags" :key="index" class="v3ti-tag">
        <slot v-if="isShot('item')" name="item" v-bind="{ name: tag, index, tag }"></slot>
    ...
    <section
      ...
      ref="contextMenu"
    >
      <div v-if="loading" class="v3ti-loading">
      ...
      <div v-if="!loading && selectItems.length > 0">
        ...
        >
          <div class="v3ti-context-item--label">
            <slot name="select-item" v-bind="item"></slot>
          </div>
          <svg
    ...
    addNew(e) {
      ...
      const keyShouldAddTag = e
        ? this.addTagOnKeys.indexOf(e.keyCode) !== -1 ||
          this.addTagOnKeys.indexOf(e.key) !== -1
        : true;
